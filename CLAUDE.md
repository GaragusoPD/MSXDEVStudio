...
- `src/renderer/` — Vue app; Pinia stores call `window.api`, components/editors are thin shells over the logic in `src/shared/`. Editor sessions are module-level maps keyed by tab path — except a `.tiles.json`, which lives in `stores/tilesetStore.ts` because the meta-tile editor writes into it and two in-memory copies would overwrite each other on save. Undo stays per editor and rebases on external change; that is safe only because painting appends.
