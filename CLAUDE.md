...
- Tests live next to their module. Non-trivial logic in shared/main gets tests; renderer correctness rides on the shared modules it delegates to — **except `renderer/src/stores`**, which vitest does include, because a store holds lifecycle state no shared module can reach. `tilesetStore.test.ts` needs no DOM: it stubs `window.api` in five lines.
